...
class RecommendationGenerator:
    ...
    def generate_recommendations(
        ...
        high_risk_companies = [
            c for c in companies_analysis 
            if c.get('final_risk_score', {}).get('final_risk_score', 0) >= self.high_risk_threshold
        ]
        low_risk_companies = [
            c for c in companies_analysis 
            if c.get('final_risk_score', {}).get('final_risk_score', 0) <= self.low_risk_threshold
        ]
    ...
    def _extract_company_analyses(self, impact_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for reg_id, reg_data in impact_results.get('regulations', {}).items():
            ...
            for company in reg_data.get('companies', []):
                # Check if company has required analysis
                if 'final_risk_score' in company and 'valuation_impact' in company:
    ...
    def _generate_company_recommendation(
        ...
        # Risk scores
        final_risk = company.get('final_risk_score', {}).get('final_risk_score', 0)
        risk_category = company.get('final_risk_score', {}).get('risk_category', 'Unknown')
